...
> **v2.20 视觉收敛**：该入口对 90% 的普通玩家是噪音，v2.10 的金色大卡过于抢眼。
> - **未登录商家态**：收进页面右上角 `⋮` 菜单，点击弹 ActionSheet，含「切换到球房管理模式」一项。页面主体不再显示大卡。
> - **已登录商家态**（`venueSession` 存在）：保留现有金色大卡（显示商家名 + 「查看球房状态 →」按钮），方便快速回到管理视图。
