...
- **字体**：思源黑体 Source Han Sans CN（开源，server 容器打包字体文件）
- **emoji**：用 unicode 文本 + Noto Color Emoji 字体兜底；服务端 emoji 渲染不稳的话，关键场景（冠军 🏆）改成图片资源
- **配色**：与 app 一致 —— `--bg-dark` `#0a0f0d` 底，`--accent` `#d4af37` 强调

### 3.2 模板结构（自上而下）

```
┌─────────────────────────────┐
│  🎱 击球帮 · 战报             │  ← 角标（72px 字 + logo emoji）
├─────────────────────────────┤
│                             │
│      九球追分 · ABC123       │  ← 比赛类型 + 房间码
│                             │
│   ┌─────┐         ┌─────┐   │  ← 1v1：双方头像左右
│   │ 头像 │   VS   │ 头像 │   │     多人：金字塔（榜首居中）
│   │ 张三 │         │ 李四 │   │
│   └─────┘         └─────┘   │
│                             │
│       7    :    5           │  ← 比分 240px 大字
...
### 3.3 多人场景模板差异
- 3-4 人比赛：取 top 3 头像，金字塔排列，第 1 居中放大
- > 4 人（罕见）：top 3 + "其他 N 人"，避免画面挤
